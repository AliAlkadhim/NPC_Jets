...
def get_bin_entries_list(filename, hist_name, n_bins):
    ...
    with open(filename, 'r') as f:
        ...
        for line_ind, line in enumerate(f_readlines):
            # line_counter +=1 
            if begin_hist_string_Paris+ hist_name in line:
                ...
                for i in range(n_bins):
                    ...
                    entry_val = f_readlines[begin_table_ind].split()[-1]
...
def main():
    ...
    #SINGLE TEST
    # pre_bins_list, pre_entries_list= get_bin_entries_list(pre_filename,AK4_NAMES[0], MAP_DICT_PARIS[AK4_NAMES[0]]['n_bins'])
    # post_bins_list, post_entries_list= get_bin_entries_list(post_filename,AK4_NAMES[0],MAP_DICT_PARIS[AK4_NAMES[0]]['n_bins'])
    ...
    for hist_ind, hist in enumerate(MAP_DICT_PARIS.keys()):
        pre_bins_list, pre_entries_list= get_bin_entries_list(pre_filename,hist, MAP_DICT_PARIS[hist]['n_bins'])
        post_bins_list, post_entries_list= get_bin_entries_list(post_filename,hist, MAP_DICT_PARIS[hist]['n_bins'])

        NPC = post_entries_list/pre_entries_list
        
        plt.step(pre_bins_list, NPC, label=MAP_DICT_PARIS[hist]['ylabel'], where='mid')
        plt.ylim(RANGE)
        plt.xlabel('$p_T$ [GeV]')
        plt.ylabel('NP C')
        plt.legend()
