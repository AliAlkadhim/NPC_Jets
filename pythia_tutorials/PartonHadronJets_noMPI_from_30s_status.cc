...
void getPartonLevelEvent( Event& event, Event& partonLevelEvent) {
  ...
  if (event[i].isFinalPartonLevel()) {
	...
	// cout << "the status code of the parton level particle in this event " << event[i].status() << endl;
...
TFile *Jetsoutput  = new TFile("NoMPI_from_Status_30s_Jetsoutput.root", "recreate");
