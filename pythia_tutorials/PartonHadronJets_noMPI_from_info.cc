...
void getPartonLevelEvent( Event& event, Info& info, Event& partonLevelEvent) {
  ...
  if (event[i].isFinalPartonLevel()) {
    ...
    // decays, its status code gets negated.
  ...
  // status codes 31 – 39 imply particles from MPI
    ...
    if (info.nMPI() == 0) {
      	cout << "the status code of the parton level particle in this event WITHOUT MPI" << event[i].status() << endl;
...
TFile *Jetsoutput  = new TFile("NoMPI_from_INFO_Jetsoutput.root", "recreate");
...
tree->Branch("phiHadronJets", &phiHadronJets, "phiHadronJets/D");
  ...
  int nEvent    =50000;
  int nListEvts = 0;// number of Event objects that you want to list (display in stdout)
  int nListJets = 0;//number of Slowjet objects you want to list (diplay)
